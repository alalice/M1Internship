...
```{r}
biobsy_N_1 <- biobsy_N[is.na(biobsy_N$diag_rejet_pbr_biopsy) == FALSE,]
```
...
for (col in names(biobsy_N_1)) {
  if(sum(is.na(biobsy_N_1[[col]]))!= 0){
    #cat(col ,":" , sum(is.na(biobsy_N_1[[col]])),"\n")
...
lenght<- length(biobsy_N$id_g)
for (col in names(biobsy_N)) {
  if(sum(is.na(biobsy_N[[col]]))!= 0){
    #cat(col ,":" , sum(is.na(biobsy_N[[col]])),"\n")
    dataframe[col] <- c(sum(is.na(biobsy_N[[col]])),sum(is.na(biobsy_N[[col]]))/lenght,"random")}
}
```  

```{r}
write.xlsx(dataframe, file = "C:\\Alice\\1-Universitée\\Stages\\M1-centrale\\DATAS\\Describe\\explore\\explore_NA.xlsx",sheetName = "biobsy", append = TRUE)
```

```{r}
write.xlsx(dataframe1, file = "C:\\Alice\\1-Universitée\\Stages\\M1-centrale\\DATAS\\Describe\\explore\\explore_NA.xlsx",sheetName = "biobsy2", append = TRUE)
```

Save the new data frames
```{r}
write.xlsx(biobsy_N_1, file = "C:\\Alice\\1-Universitée\\Stages\\M1-centrale\\DATAS\\NewDATA\\ALL_VARIABLES_KITGENIE_CLEAN.xlsx",sheetName = "biobsy", append = TRUE,)
```
