...
```{r}
write.xlsx(dataframe, file = "C:\\Alice\\1-Universitée\\Stages\\M1-centrale\\DATAS\\Describe\\explore\\explore_NA.xlsx",sheetName = "complications_graft", append = TRUE)
```
               
```{r}
compliG_N_A <- compliG_N[,-c(1,84,85,86,87,88,89,90,91,92,93,94,95)]
for (col in names(compliG_N_A)) {
  compliG_N_A[[col]][is.na(compliG_N_A[[col]])] <- -2
}
```
